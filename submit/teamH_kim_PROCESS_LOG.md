...
### [#23] app 통합 디지털트윈 3D 시각화 — 위성/DEM/건물 동일 bbox 정합·지형 기복·DEM 침수 판정
- 작성자(팀원): kim
- 목표: 통합 app(Jin)에 들어갈 부산 해운대권 3D 디지털트윈을 실제 공개자료로 정합 재구축 — 위성·지형/수심·건물을 동일 좌표로 맞추고, DEM 기반으로 지형 기복과 침수를 판정.
- 에이전트에게 시킨 것(실제 프롬프트 핵심 인용):
  > "위성 지형 수심 건물 모형 위치 정확하게 매치시키고" / "지형 높이에 따른 지형 수심 변화가 3d에 나타나야해 지금 다 평면으로 보임" / "바다인데 아직 육지처럼 올라와 있는 부분이 좀 있네"
- 사용한 기법: (b 외부 데이터 — ESRI World Imagery 위성, AWS Terrarium 표고/수심, OSM Overpass 건물), (c 재사용 산출물 — 공통 bbox 모듈 + 재현 베이크 스크립트)
- 결과: 위성·DEM·건물을 **단일 bbox·동일 등각투영**으로 재베이크해 건물–지도 어긋남 해결. Terrarium 연안 0m 문제를 **경계 플러드필 바다마스크 + 해안거리 합성수심**으로 실제 수심 복원(바다셀 1,757→55,490). 지면을 DEM으로 정점 변위(육지 표고·해저 수심)해 평면→입체, 건물은 지형 위 안착(과장 1.6× 일관). 침수는 **실제 DEM에서 η>지반고**로 판정(침수심=η−지반고).
- 막힘 → 해결: Terrarium 연안 0m로 절반이 육지처럼 처리됨 → 바다마스크+합성수심으로 해결. 수직 과장이 육상 침수면을 띄움 → 정점 DEM 샘플로 육상은 실수면·외해만 과장 분리.

### [#24] GPU 물 셰이더(곡선 쓰나미 파면·침수심별 색) + 2D 모델맵(fort.14 maxele)
- 작성자(팀원): kim
- 목표: 물을 GPU로 현실감 있게(곡선 파면·침수심별 색) 표현하고, 모델 결과를 2D maxele 지도로 표출.
- 에이전트에게 시킨 것(실제 프롬프트 핵심 인용):
  > "물 시뮬레이션 퀄리티는 비주얼적으로 업그레이드 … gpu 기반으로 자연스럽고 현실적이도록" / "일직선으로 퍼지는데 … 부채꼴로 들어오거나 휘어서" / "침수 범위 색도 기본 물높이에서는 없도록" / "2D … fort14 격자 영역대로 … max 수위만 한장"
- 사용한 기법: (a 서브에이전트 — three.js GPU 물 셰이더 레시피·2D maxele 빌드 워크플로), (b 지역 DEM 마스크)
- 결과: 단일 `ShaderMaterial`로 다중 Gerstner 스웰 + **먼 발생원 방사거리 기반 곡선(부채꼴) 쓰나미 파면**(드로다운→마루→침수 지속→후퇴), 침수심별 위험색(외해=수심 블루/육상=침수색). **침수색은 정상 해수면 위 육지가 쓰나미로 잠길 때만**(기본 물높이 잠김은 바다색). 2D는 **실측 지역 DEM 마스크**로 실제 해안선 기반 maxele 필드 + fort.14 도메인 경계·진앙·대상지 마커. (per-fragment GPU 전환)
- 막힘 → 해결: 흰 충격파 띠가 인공적 → 포말 완화·곡선 파면으로 자연화. 기본 물높이에서 침수색이 깔리던 문제 → 침수 기준을 정상 해수면 위로 변경.
